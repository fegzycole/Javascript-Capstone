...
class Level extends Phaser.Scene {
  ...
  create() {
    ...
    gameState.bgColor = this.add.rectangle(0, 0, config.width,
      config.height, 0x00ffbb).setOrigin(0, 0);
    this.createStars();
  ...
  createParallaxBackgrounds() {
    ...
    const bg1Width = gameState.bg1.getBounds().width;
    const bg2Width = gameState.bg2.getBounds().width;
    // const bg3_width = gameState.bg3.getBounds().width;

    gameState.bgColor.setScrollFactor(0);
    gameState.bg1.setScrollFactor((bg1Width - windowWidth) / (gameWidth - windowWidth));
